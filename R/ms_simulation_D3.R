...
#     return(names(sort(apply(submat, 1, sum))))
#   }else if (identical(as.numeric(table(submat)),c(5,4,8,8))){
#     ox=names(sort(apply(submat, 1, sum)))
#     if (submat[ox[1],ox[2]]==as.numeric(names(table(submat))[2])){
#       return(ox[c(1,2,3,4,5)])
#     }else{
#       return(ox[c(1,3,2,4,5)])
#     }
#   }
# }
...
single_trees<-unlist(lapply(which(rep != "NULL"), function(x) rep[[x]]$trees[[1]]))
...
# topologiesDfoil<-apply(pretopologiesDfoil,c(1,2), function(x) da[as.character(da$na) == as.character(x),2])
#
# results2<-as.data.frame(do.call("rbind",mclapply(as.data.frame(t(topologiesDfoil)),function(x) Dfoiler(sites,x),mc.cores = N_CORE)))
# results2$TF<-apply(results2,1,function(x) is_true_Dfoil(do.call('paste',c(as.list(x[c(20:23)]),sep="")),as.character(x[24])))
#
#
# pretopologiesDfoil<-getquintet(t(combn(tr$tip.label,5)))
# topologiesDfoil<-unlist(lapply(pretopologiesDfoil, function(x) da[as.character(da$na) == as.character(x),2]))
#
# topologiesDfoil<-unlist(lapply(pretopologiesDfoil, function(x) which(da$na == x)))
# results2<-Dfoiler(sites,topologiesDfoil)
#
#
#
# results2$TF<-apply(results2,1,function(x) is_true_Dfoil(do.call('paste',c(as.list(x[c(20:23)]),sep="")),as.character(x[24])))
#
# table(results2$TF)
# results2[which(results2$TF == "New_pattern"),]
#
# results2[which(results2$TF == "TP"),c(1:24)]
# results2[which(results2$TF == "FN"),c(1:24)]
# results2[which(results2$TF == "FP"),c(1:24)]
# results2[which(results2$TF == "TP"),c(1:24)]
#
# table(results2[which(results2$TF == "FP"),c(1:24)]$EV)
#
#
# results2$pattern<-do.call('paste',c(as.list(results2[c(20:23)]),sep=""))
#
# by(results2, results2$EV, function(x) table(x$pattern))
#
#
# results2[which(results2$EV == "P3-P2"),]
#
#
#
# Dfoiler(top)
