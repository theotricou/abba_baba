...
## -> compute the same proportion if only considering
...
SelectIntrogressionDistanceEffect<-function(tree, xy, alpha, lwd) {
...
GetGhostAndNoGhostPerQuartet<-function(event, allscenario) {
...
ComputeAll<-function(N=20, pextinction=runif(1), Nreal=sample(seq(20,100,by=20),1), alpha=sample(c(0,1,10,100,1000),1), Nintrog=100, fileout="res_output") {
	...
	# Res<-c(VALINFOS, TREEINFOS, QUATINFOS, ALLRESULTS)
	# names(Res)<-c("pext", "alpha", "Nintrog","Nextant","Nreal","Ntotal","Colless","Nbquartets", paste(rep(seq(0,0.9,by=0.1),4),rep(c("PropIntrogDetected","PropErroneousInterp.mean", "PropErroneousInterp.sd", "PropErroneousInterp.median"), each=10),sep="-"))
	# return(Res)
}



ComputeAll_EffectExtinct<-function(tree, alpha=0, Nintrog=1000) {
    tr<-tree
    extant<-paste("t",1:N, sep="") #easy because of the way rphylo is naming tips.
    extant.tr<-keep.tip(tr, extant)
    coll<-colless(extant.tr) ##how much is the tree balanced
    ...
    events<-replicate(Nintrog, SelectIntrogressionDistanceEffect(tr, xy, alpha=alpha, lwd=1))
    result<-sapply(events, GetDistribDistanceToOutgroup, allscenario=ScenrarioPerQuartet, simplify=FALSE)
    res<-do.call(rbind, lapply(result, function(x) unlist(lapply(x, function(y) sum(y>=0)))))
    print(res)
    res2<-apply(res,1,function(x) x[2]/sum(x))
    meanres2<-mean(res2, na.rm=TRUE)
    # Res<-c(VALINFOS, TREEINFOS, QUATINFOS, ALLRESULTS)
    # names(Res)<-c("pext", "alpha", "Nintrog","Nextant","Nreal","Ntotal","Colless","Nbquartets", paste(rep(seq(0,0.9,by=0.1),4),rep(c("PropIntrogDetected","PropErroneousInterp.mean", "PropErroneousInterp.sd", "PropErroneousInterp.median"), each=10),sep="-"))
    # return(Res)
    return(meanres2)
...
#ComputeAll(20,pext,20,0,100,"fichier-de-sortie")

#
# #!/usr/bin/env Rscript
# # Théo
#
# source("/mnt/mydatalocal/scr.R")
#
# files_out<-paste('test_AAAA_',  seq(BBBB,BBBB+100,1), sep="")
#
# lapply(files_out, function(x) ComputeAll(20,0,20,0,100,x))
#
#
#
#
# for i in 0 0.3 0.6 0.9; do
#   for j in 1 51 101 151 201 251 301 351 401 451; do
#     mkdir output_$i\_$j
#     cd output_$i\_$j
#     echo $i $j
#     sed "s/AAAA/$i/g" ../launcher.R > temp
#     sed -i "s/BBBB/$j/g" temp
#     screen -Sdm output_$i\_$j Rscript temp
#     cd ..
#   done
# done
#
...
# ####TEMP
# ####COMPARE THEO
# TT<-read.tree("ext9_spe_tree")
# N<-20
# Nreal<-40
# alpha<-0
# Nintrog<-500
# tr<-TT
# tr$tip.label[grep("s>",tr$tip.label)]<-1:N
# tr$tip.label[grep("u>",tr$tip.label)]<-(N+1):Nreal
# tr$tip.label<-paste("t",tr$tip.label, sep="")
# tr<-read.tree(text=write.tree(tr))
# extant<-paste("t",1:N, sep="") #easy because of the way rphylo is naming tips.
# extant.tr<-keep.tip(tr, extant)
# quatuors <- getquatuors(extant.tr)

# #get transfers simulated by Theo
# trans<-lapply(strsplit(readLines("ext9_list_tr")[1:500]," "), function(x,X) x[c(2,6)])
# trans2<-lapply(trans, function(x,tree) match(x,c(tree$tip.label, tree$node.label)),tree=TT)
# trans3<-lapply(trans2, function(x,tree) apply(tree$edge[match(x,tree$edge[,2]),],1,paste,collapse="-"), tree=tr)
# eventsTHEO<-unlist(lapply(trans3,paste, collapse="->"))
# #get result from quaturos
# temp<-read.table("ext9_error_by_quatuor", header=T)
# quatuorsTHEO<-lapply(strsplit(as.character(temp$quat),"_"), as.numeric)
# quatuorsTHEO2<-apply(do.call(rbind, quatuorsTHEO),1,function(x, tree) paste(tree$tip.label[x], collapse="|"), tree=tr)
# quatuors2<-apply(quatuors,1,paste,collapse="|")
# NewOrderQuat<-match(quatuorsTHEO2, quatuors2)
# plot(tr, show.tip.label=FALSE)
# xy<-get("last_plot.phylo", envir = .PlotPhyloEnv)
# 	#get all transfers that would make each quartet having a significant Dstat
# ScenrarioPerQuartet<-apply(quatuors, 1, events.donor.receiver, tr=tr, xy=xy)
# 	#Simulate Nintrog introgression events
# #events<-replicate(Nintrog, SelectIntrogressionDistanceEffect(tr, xy, alpha=alpha))
# events<-eventsTHEO
# noghost<-unlist(lapply(ScenrarioPerQuartet, function(x,y) sum(is.element(y, x$transfersinvolved$NOGHOST)),y=events))
# ghost<-unlist(lapply(ScenrarioPerQuartet, function(x,y) sum(is.element(y, x$transfersinvolved$GHOST)),y=events))
# noghost2<-noghost[NewOrderQuat]
# ghost2<-ghost[NewOrderQuat]

# plot(ghost2/(ghost2+noghost2), temp$Ghost/(temp$Ghost+temp$Ingroup))
# par(mfrow=c(2,1))
# par(mar=c(0,0,0,0))
# tr<-rphylo(20,1,0, fossils=TRUE)
# plot(tr, show.tip.label=FALSE, edge.width=3)
# xy<-get("last_plot.phylo", envir = .PlotPhyloEnv)
# replicate(20, SelectIntrogressionDistanceEffect(tr, xy, alpha=0, lwd=2))

# plot(tr, show.tip.label=FALSE, edge.width=3)
# xy<-get("last_plot.phylo", envir = .PlotPhyloEnv)
# replicate(20, SelectIntrogressionDistanceEffect(tr, xy, alpha=1000, lwd=2))
