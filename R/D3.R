...
leaf_descendants <- function(leaf, comp_leaf) {
  ...
  des_leaf <- c(first_son_leaf,Descendants(spe, as.integer(first_son_leaf), type = "all"))
  return(des_leaf)
}
...
position_D_in_tree<-function(p1,p2,p3){
  if (donor %in% leaf_descendants(p3,p2)){return("P3")}
  else if (donor %in% leaf_descendants(p2,p1)){return("P2")}
  else if (donor %in% leaf_descendants(p1,p2)){return("P1")}
  else if (donor %in% Descendants(spe,mrca.phylo(spe, c(p1, p3)), type="all")){return("N1")}
  else{return("O")}
}

position_R_in_tree<-function(p1,p2,p3){
  if (recip %in% leaf_ancestors(p3,p2)){return("P3")}
  else if (recip %in% leaf_ancestors(p2,p1)){return("P2")}
  else if (recip %in% leaf_ancestors(p1,p2)){return("P1")}
